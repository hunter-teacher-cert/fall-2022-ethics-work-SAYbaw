...
def purchase_economy_pair(plane, economy_sold, pair, name):
    """
    Purchase regular economy seats. This will ensure at least one passenger
    requiring assistance is seated next to someone. As long as there are sufficient seats
    available, store the name and number of seats purchased in the
    economy_sold dictionary and return the new dictionary
...
def fill_plane(plane):
    ...
    while total_seats > 1:
        ...
        if r > 55: #NOTE: changed the probability
      ...
      # HERE IS WHERE MY ORIGINAL ALGORITHM FAILED!! I tried several different logics, but I could could not get groups and pairs to seat together
      # randomly, so I decided to kill purchase_economy_block() so my code would work in the spirit of equity for the assignment.
        else:
            #economy_sold = purchase_economy_block(plane, economy_sold, 1 + random.randrange(max_family_size),
            #                                      "u-%d" % u_number)
            #u_number = u_number + 1
            pair = 2
            economy_sold = purchase_economy_pair(plane, economy_sold, pair, "u-%d" % u_number)
            u_number = u_number + 1
